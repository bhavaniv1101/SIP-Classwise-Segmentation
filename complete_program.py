...
class SegDataset(torch.utils.data.Dataset):
    def __init__(self, patchify: bool = False, patch_size: int = None, step: int = None,
                 root: str = 'C:/Users/bhava/Documents/SIP AAI-06/UNet_Aerial_Segmentation/100_dataset',
                 training: bool = True, transform: bool = None, img_names: str = None, mask_names: str = None
                 ) -> None:
        """
        ...
        self.BGR_classes = {'Water': [255, 0, 0],  # blue
        ...
        # list of classes
        self.bin_classes = ['Water', 'Pool',
                            'Land', 'Dirt', 'Gravel', 'Rocks', 'Road', 'Paved area',
                            'Grass', 'Vegetation', 'Tree', 'Bald-tree',
                            'Building', 'Roof', 'Wall', 'Window', 'Door', 'Fence', 'Fence-pole',
                            'Car', 'Bicycle', 'Person', 'Dog',
                            'AR-marker', 'Background', 'Obstacle', 'Conflicting', 'Unlabeled']
        ...
        def is_single_color(image, color: list) -> bool:
            ...
            """
            pixels = np.all(image == color, axis=-1)  # array of bool values - True if pixel == color, False otherwise
            for row in pixels:
        ...
        def patch_image(img_path: str, img_num: int, x_size: int, y_size: int, step: int,
                        check_colors: bool = False, color_list: list = None) -> list:
            """
            ...
            for start_x in range(0, image.shape[1], step):
                for start_y in range(0, image.shape[0], step):
                    ...
                    patch = image[start_y: start_y + y_size, start_x: start_x + x_size]
                        ...
                        if check_colors:
                            ...
                            for color in color_list:
                                if is_single_color(patch, color):
                                    ...
                                    remove_list.append(f'patch_img_{img_num}_{patch_num}.jpg')
                                    keep = False
                                    break
        ...
        def save_patches() -> None:
            ...
            for img_num, img_path in enumerate(self.IMG_NAMES):
                ...
                os.chdir('C:/Users/bhava/Documents/SIP AAI-06/UNet_Aerial_Segmentation/patched/images')
                if not (img_path.startswith(
                        'C:/Users/bhava/Documents/SIP AAI-06/UNet_Aerial_Segmentation/100_dataset/images/0')):
                    remove_list += patch_image(img_path, img_num, patch_size, patch_size, step,
                                               check_colors=True, color_list=[[255, 255, 255]])
            ...
            for mask_num, mask_path in enumerate(self.MASK_NAMES):
                ...
                os.chdir('C:/Users/bhava/Documents/SIP AAI-06/UNet_Aerial_Segmentation/patched/masks')
                if not (mask_path.startswith(
                        'C:/Users/bhava/Documents/SIP AAI-06/UNet_Aerial_Segmentation/100_dataset/masks/0')):
                    remove_list += patch_image(mask_path, mask_num, patch_size, patch_size, step,
                                               check_colors=True, color_list=[[0, 0, 0], [41, 169, 226], [246, 41, 132],
                                                                              [58, 221, 254]])
                else:
            ...
            remove_list.append('patch_img_99_24.jpg')  # to make data size 1500 instead of 1501
            ...
            for name in remove_list:
                img_path = 'C:/Users/bhava/Documents/SIP AAI-06/UNet_Aerial_Segmentation/patched/images/' + name
                if os.path.exists(img_path):
                    os.remove(img_path)
                mask_path = 'C:/Users/bhava/Documents/SIP AAI-06/UNet_Aerial_Segmentation/patched/masks/' + name
        ...
        if patchify:
            save_patches()
            self.IMG_NAMES = sorted(glob('C:/Users/bhava/Documents/SIP AAI-06/UNet_Aerial_Segmentation/patched/images/*'))
            self.MASK_NAMES = sorted(glob('C:/Users/bhava/Documents/SIP AAI-06/UNet_Aerial_Segmentation/patched/masks/*'))
    ...
    def __getitem__(self, idx: int):
        ...
        cls_mask[mask == self.BGR_classes['Paved area']] = self.bin_classes.index('Paved area')
        ...
        cls_mask[mask == self.BGR_classes['Bald-tree']] = self.bin_classes.index('Bald-tree')
        ...
        cls_mask[mask == self.BGR_classes['Fence-pole']] = self.bin_classes.index('Fence-pole')
        cls_mask[mask == self.BGR_classes['Car']] = self.bin_classes.index('Car')
        cls_mask[mask == self.BGR_classes['Bicycle']] = self.bin_classes.index('Bicycle')
        ...
        cls_mask[mask == self.BGR_classes['AR-marker']] = self.bin_classes.index('AR-marker')
        cls_mask[mask == self.BGR_classes['Background']] = self.bin_classes.index('Background')
        cls_mask[mask == self.BGR_classes['Obstacle']] = self.bin_classes.index('Obstacle')
        cls_mask[mask == self.BGR_classes['Conflicting']] = self.bin_classes.index('Conflicting')
...
if __name__ == '__main__':
    ...
    dataset = SegDataset(patchify=False,
    ...
    for epoch in range(N_EPOCHS):
        ...
        print(' epoch {} - loss : {:.5f} - acc : {:.2f} - val loss : {:.5f} - val acc : {:.2f}'.format(
            epoch + 1, np.mean(loss_list), np.mean(acc_list), np.mean(val_loss_list), np.mean(val_acc_list)))
